...
exports.handler=async function(event){
  ...
  try{
    const m=await blobStore('cleaning3-publish-manifests');
    const latest=await m.get('latest',{type:'json'});
    const urls=(latest&&latest.urls)||[];
    ...
    const checks=await Promise.all(sample.map(async u=>{
      try{
        const r=await fetch(u+'?audit='+Date.now(),{redirect:'follow',headers:{'Cache-Control':'no-cache'}});
        const t=await r.text();
        const isHtml=/<html[\s>]/i.test(t)||/<title[\s>]/i.test(t)||/<!doctype html/i.test(t);
        return {url:u,ok:r.ok&&isHtml,status:r.status};
      }catch(e){return {url:u,ok:false,status:0,error:e.message}}
    }));
    return json(200,{pass:checks.length>0&&checks.every(x=>x.ok),total:urls.length,sample:checks});
